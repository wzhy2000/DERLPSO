...
class PSO():
    ...
    def function(self, X):
        ...
        # mse = np.mean((self.actual_data - predicted_data) ** 2) + 0.0001 * math.sqrt(sum(i ** 2 for i in X))
        return mse
    ...
    def init_population(self):
        for i in range(self.pN):
            ...
            # lower_bound = (self.lower - mean) / 1.0
            # upper_bound = (self.upper - mean) / 1.0
            # for i in range(self.pN):
            #     for j in range(self.dim):
            #         self.X[i][j] = truncnorm.rvs(lower_bound, upper_bound, loc=mean, scale=1.0)
            #         self.V[i][j] = truncnorm.rvs(lower_bound, upper_bound, loc=mean, scale=1.0)
            # for j in range(self.dim):
            #     self.X[i][j] = random.uniform(self.lower, self.upper)
            #     self.V[i][j] = random.uniform(self.lower, self.upper)
            # for j in range(self.dim):
            if (i <= self.pN / 2):
    ...
    def iterator(self):
        for t in range(self.max_iter):
            ...
            #     min_values = sorted_fitness[:int(0.5 * self.pN)]
            #     indices = [i for i, x in enumerate(fitness) if x in min_values]
            #     for i in indices:
            #             # for j in range(self.dim):
            ...
            # for i in range(2, currentTotalLayer):
            for i in range(currentTotalLayer - 1, 1, -1):
                for j in self.layers[i]:
                    ...
                    # for m in range(0, len(self.X[j])):
                    #     if self.X[j][m] > self.upper:
                    #         self.X[j][m] = 10
                    #         self.V[j][m] = -self.V[j][m]
                    #     elif self.X[j][m] < self.lower:
                    #         self.X[j][m] = -10
                    #         self.V[j][m] = -self.V[j][m]
            ...
            for k in self.layers[1]:
                ...
                # X1 = self.X[random.choice(self.layers[0])]
                # X2 = self.X[random.choice(self.layers[0])]
                # if (X1 == X2):
                #     print("true")
                ...
                # for m in range(0, len(self.X[k])):
                #     if self.X[k][m] > self.upper:
                #         self.X[k][m] = 10
                #         self.V[k][m] = -self.V[k][m]
                #     elif self.X[k][m] < self.lower:
                #         self.X[k][m] = -10
                #         self.V[k][m] = -self.V[k][m]
            ...
            # if (t +1 % 10 == 0):
            #     for i in range(50,self.pN):
            #         for j in range(self.dim):
            #             self.X[i][j] = np.exp(np.log(self.lower) + np.log(self.upper / self.lower) * np.random.uniform(0, 1,
            #                                                                                                            1)) * np.random.choice(
            #                 [-1, 1], 1)
            #             self.V[i][j] = np.exp(np.log(self.lower) + np.log(self.upper / self.lower) * np.random.uniform(0, 1,
            #                                                                                                            1)) * np.random.choice(
            #                 [-1, 1], 1)
