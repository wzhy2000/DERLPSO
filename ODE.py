...
class Model():
    def __init__(self, ODEModel, paramNum, data, time, particleNum=100, maxIter=200, layersList=[4, 6, 8, 10], upper=10, lower=1e-10, threshold=1e-04):
    ...
    def MSELoss(self, X):
        predictedData = odeint(self.ODEModel, self.initial, self.time, args=tuple(X))
    ...
    def initParticles(self):
        for i in range(self.particleNum):
            if (i <= self.particleNum / 2):
                ...
                self.X[i] = [random.uniform(-self.upper, self.upper) for _ in range(self.paramNum)]
                self.V[i] = [random.uniform(-self.upper, self.upper) for _ in range(self.paramNum)]
